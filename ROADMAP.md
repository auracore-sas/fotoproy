...
- [x] **F1.7** **Cámara** (`expo-camera`): `app/(app)/capture.tsx` — preview pantalla completa, **flash 3 modos (OFF/AUTO/ON)**, **zoom ＋/－** (0–1, etiqueta aproximada ×1–×4), permisos con estado denegado + abrir ajustes; captura instantánea (GPS no bloquea el disparo) → preview con estampa → guardar/descartar. (Enfoque al tocar: pendiente de soporte de la API de expo-camera.)
