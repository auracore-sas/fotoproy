...
> ✅ **Backend F3.1/F3.3/F3.5 (API) completado (2026-09-05)**: plans (presign PUT + create idempotente/409, list, findOne, thumbnail JPEG de planos-imagen, org-scoped), pins (POST /pins con validación plan/foto mismo proyecto + página, list por plan con resumen de foto firmada) y comments (POST/list con authorName) — verificado E2E por curl (incl. 403 técnico, 404 cross-org, 409 conflicto, 400 página inválida). **Pendiente: lado móvil (F3.2 visor/toque, F3.3 UX pin, F3.5 UI + F3.6 cola offline).**

- [x] **F3.0** **Spike técnico** — ⚠️ Decisión (2026-09-05): **WebView + pdf.js para PDF multipágina** (funciona en Expo Go; `react-native-pdf` exige dev client/EAS → se re-evalúa en F4). El MVP prioriza **planos-imagen** (F3.2) y el PoC de toque→(x%,y%) se entrega dentro del visor F3.2 antes de habilitar PDF.
...
- [ ] **F3.2** **Plans móvil**: selector de archivos (`expo-image-picker`/`expo-document-picker`), subida con progreso, visor a pantalla completa (imagen multipágina: paginación swipe; PDF: según spike F3.0), lista de planos del proyecto (solo ADMIN/SUPERVISOR suben).
- [ ] **F3.3** **Pines API + móvil**: API ✅ (`POST /pins` idempotente, `GET /plans/:id/pins` con foto+thumbnail firmada) — pendiente UX móvil: en el visor, **tocar un punto** → opciones "usar foto existente" o "tomar foto ahora" → se coloca el pin y se abre la foto.
