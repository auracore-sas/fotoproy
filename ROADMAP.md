...
> **Estado al 2026-09-15:** F4.1 y F4.2 ✅ (mergeados a `main`, commit `d75132b`) · **F4.3 ✅** (pulido UX/offline).
> **Pendiente inmediato (validación del usuario)**: abrir un enlace creado desde la app en el navegador del teléfono y confirmar que se ven las fotos con su estampa, la página individual y el mensaje al revocar. Sin esa validación no se cierra el DoD de F4.1/F4.2.
> **Siguiente bloque de trabajo**: **F4.4 QA** (matriz iOS/Android + E2E). **F4.5–F4.7** después. Sigue bloqueado por terceros: **F2.4 despliegue** (credenciales Cloudflare R2 + dominio propio; hasta entonces el enlace usa la IP de LAN en dev).
