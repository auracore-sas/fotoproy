...
- [x] **F1.7** **Cámara** (`expo-camera`): `app/(app)/capture.tsx` — preview pantalla completa, flash on/off, permisos con estado denegado + abrir ajustes; captura → preview con estampa → guardar/descartar. (Enfoque al tocar: pendiente de soporte de la API de expo-camera.)
