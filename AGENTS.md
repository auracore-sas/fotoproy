...
**Estado actual:** Fase 0 ✅ (M0) · Fase 1 ✅ (M1) · **Fase 2 ✅ (M2)** — storage S3-compatible (MinIO dev / R2 prod) con pre-signed URLs, photos idempotentes org-scoped con thumbnails JPEG, motor de sync offline-first (FIFO + backoff, NetInfo, UI de pendientes, galería en línea) — validado en dispositivo. **Fase 3 ✅ (M3, tag `v0.2.0-m3`)**: planos (subida/progreso, visor con zoom, lista), **anclaje de fotos por toque → pines (x%,y%)** local-first con sync, pines sobre el plano, fotos del plano, filtros de galería (autor/fecha/plano) y comentarios offline-first — validado en dispositivo (incl. cola offline que sube sola al reconectar; v1.24). **Fase 4 🚧 en curso**: **F4.1 ✅** enlaces de solo lectura (token hasheado, expiración, revocar, `POST/GET/DELETE /shares`, pantalla “Compartir avance”) y **F4.2 ✅** vista web server-rendered (validados en dispositivo 2026-09-16), **F4.3 ✅** pulido UX/offline, **F4.3.1 ✅** rediseño visual “blueprint industrial”, **F4.3.2 ✅** planos: flujo de anclaje explícito (botón + puntero arrastrable + “Fijar aquí”), zoom tipo galería con foco, quitar del plano (soft-remove `removedAt` + cola `pin_remove`) y **planos offline** (caché en `project_plans.localUri`) y **F4.3.3 ✅** planos y anclajes en la vista web compartida (sección “Planos de la obra”, página de plano con marcadores numerados, medios por proxy). Spec en [docs/shares.md](docs/shares.md). **Despliegue (F2.4) ✅ — 2026-09-16**: la API corre en **Dokploy** (servidor propio) en **`https://fotoproy.apx5.com`**, con el PostgreSQL 16 y el MinIO del mismo servidor (`minio:9000` interno, API S3 público `minio-api.apx5.com`) y migraciones al arrancar. Los enlaces de solo lectura funcionan desde fuera de la LAN (verificado end-to-end). **F4.5 ✅**: rate limiting por IP (600/120/1200 por minuto), topes de subida verificados contra el objeto real, EXIF/GPS fuera, cuerpos estrictos y `pnpm check:secrets`. **F4.6 🚧**: **APK compilado en local y funcionando en un Samsung A13** (`scripts/build-apk.sh`, arm64+armeabi-v7a, firmado con keystore de debug, API de producción verificada dentro del bundle); faltan las cuentas de EAS/Apple/Google para TestFlight y Play. **Pendiente para el próximo día**: **F4.4 QA** (matriz iOS/Android, org-scoping, bugs), cerrar F4.6 con cuentas o más APKs locales, y el endurecimiento del servidor (rotar secretos, usuario dedicado de MinIO, `minio-api.apx5.com` en DNS only, PostgreSQL con usuario dedicado). Ver ROADMAP para detalle.
...
- [ ] Toda entrada de la API valida con schemas de `@fotoproy/shared`; los **cuerpos** rechazan campos desconocidos (no los ignoran). Al añadir un campo al payload del móvil hay que añadirlo también al schema o la petición falla con 400.
